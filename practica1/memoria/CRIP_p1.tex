...
En esta sección se nos pide, dado un $n = p \cdot q$ siendo $p$ y $q$ dos números primos, implementar algoritmos para obtener un $r$ tal que $r^2 \cong a \mod p$, siendo $a$ residuo cuadrático de $p$, es decir, su símbolo de Jacobi vale uno, y por otro lado obtener no solo una raiz, si no todas.
...
\begin{itemize}
	\item $0$ si $a \cong 0 mod p$.
	\item $1$ si $a \not \cong mod p$ y para un entero $x$, $a \cong x^2 mod p$, por lo que es un residuo cuadrático.
	\item $1$ si $a \not \cong mod p$ y no existe un $x$, $a \cong x^2 mod p$.
\end{itemize}
